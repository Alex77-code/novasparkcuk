...
async function approveDelivery(event){
 ...
 await supabaseRequest('events',{method:'POST',body:JSON.stringify({organization_id:org.id,event_type:approved?'CLIENT_DELIVERY_APPROVED':'CLIENT_DELIVERY_REJECTED',source:'client-approval-gate',payload:{task_id:task.id,decision:body.decision,comment:body.comment||null,approved_at:now}})});
 return {ok:true,task_id:task.id,decision:body.decision,approved_at:now};
}
exports.handler=async event=>{if(event.httpMethod!=='POST')return json(405,{error:'METHOD_NOT_ALLOWED'});try{const user=await verifyUser(event.headers.authorization||event.headers.Authorization);if(!user)return json(401,{error:'AUTHENTICATION_REQUIRED'});const result=await approveDelivery(event);return json(result.error?400:200,result);}catch(e){console.error(e);return json(500,{error:'CLIENT_APPROVAL_GATE_FAILED',message:e.message});}};
