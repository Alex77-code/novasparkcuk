...
async function requireOwner(event, organizationId, options={}){
 ...
 const controls=await supabaseRequest(`system_controls?organization_id=eq.${encodeURIComponent(org)}&select=emergency_stop&limit=1`);
 const emergencyStop=Boolean(controls?.[0]?.emergency_stop);
 if(emergencyStop&&!options.allowEmergencyStop)return {ok:false,response:json(423,{error:'EMERGENCY_STOP_ACTIVE'})};
 return {ok:true,user,organizationId:org,role,emergencyStop};
}
