...
Respect approvals: low-risk planning can proceed; medium/high-risk spending, financial transactions, contracts, destructive changes and sensitive outbound actions require owner approval.
...
projects is an array of {name, objective}. tasks is an array of {title, description, agent_key, priority, risk, approval_required}.`;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (process.env.NOVA_OWNER_EMAIL && user.email !== process.env.NOVA_OWNER_EMAIL) {
    ...
    const [goals, tasks, agents, controls, memory, integrations] = await Promise.all([
    ...
    const context = { organization: org, controls: controls?.[0] || null, goals, tasks, agents, memory, integrations };
    ...
    let plan;
    try { plan = JSON.parse(outputText); } catch {
      plan = { intent: 'analysis', summary: outputText, projects: [], tasks: [], approvals: [], risks: [], next_actions: [] };
    }

    const goal = plan.target ? await supabaseRequest('goals', {
      method: 'POST',
      headers: { Prefer: 'return=representation' },
      body: JSON.stringify({
        organization_id: org.id,
        title: String(plan.intent || 'CEO Objective').slice(0, 200),
        objective: command,
        target_value: typeof plan.target === 'number' ? plan.target : null,
        target_currency: org.currency,
        kpis: [],
        strategy: { summary: plan.strategy || null },
        forecast: plan.forecast || {}
      })
    }) : [];

    const createdTasks = [];
    for (const task of Array.isArray(plan.tasks) ? plan.tasks.slice(0, 30) : []) {
      ...
      const rows = await supabaseRequest('tasks', {
        ...
        body: JSON.stringify({
          ...
          status: approval ? 'WAITING_APPROVAL' : 'PLANNED'
        })
      });
    ...
    await supabaseRequest('audit_logs', {
      ...
      body: JSON.stringify({
        organization_id: org.id,
        actor_type: 'OWNER_CEO',
        actor_id: user.id,
        action: 'CEO_COMMAND_EXECUTED',
        resource_type: 'goal',
        resource_id: goal?.[0]?.id || null,
        metadata: { command, model: MODEL, duration_ms: Date.now() - started, task_count: createdTasks.length }
      })
    });

    return json(200, { ok: true, plan, goal: goal?.[0] || null, tasks: createdTasks, model: MODEL });
  } catch (error) {
    console.error(error);
