...
async function qualifyLeads(){
 ...
 for(const lead of leads||[]){
  ...
  await supabaseRequest(`leads?id=eq.${lead.id}&organization_id=eq.${org.id}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({status,qualification_score:score,updated_at:new Date().toISOString()})});
  if(status==='QUALIFIED') await supabaseRequest('events',{method:'POST',body:JSON.stringify({organization_id:org.id,event_type:'LEAD_QUALIFIED',source:'lead-qualification-engine',payload:{lead_id:lead.id,score}})});
